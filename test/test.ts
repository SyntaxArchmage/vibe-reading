...
// === Test (remote): Harness rejects invalid schemas ===
console.log("\nTest (remote): Harness schema validation (negative cases)");
...
assert(harnessOutBadLine.includes("start_line") && (harnessOutBadLine.includes(">= 1") || harnessOutBadLine.includes("≥ 1")), "Harness detects start_line < 1");
...
// === Test (remote): Enrich with start_line disambiguation ===
console.log("\nTest (remote): Enrich with start_line disambiguation");
...
// === Test (remote): Auto-enrich integration (analyze → auto-enrich → harness) ===
{
  console.log("\nTest (remote): Auto-enrich pipeline integration");
  ...
  const autoEnrichOut2 = run(`npx tsx auto-enrich.ts ${FIXTURE_DIR}`);
  assert(autoEnrichOut2.includes("entities enriched"), "Auto-enrich completes successfully");
